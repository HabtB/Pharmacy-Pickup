...
class MedspacyMedicationParser:
    ...
    def _looks_like_medication(self, word):
        """
        Determine if a word looks like a medication name using linguistic patterns
        ...
        """
        # Common pharmaceutical suffixes and patterns
        pharma_suffixes = ['pril', 'statin', 'olol', 'pine', 'zole', 'mycin', 'cillin', 'ide']
        pharma_prefixes = ['hydro', 'metro', 'amlod', 'atorv', 'lisino', 'omepra']
        ...
        for prefix in pharma_prefixes:
            if word_lower.startswith(prefix):
                return True
        
        # Check for typical medication name patterns
        # Many medications have specific letter combinations
        if any(combo in word_lower for combo in ['pril', 'statin', 'mycin', 'cillin']):
            return True
        
        # If word is long and alphabetic, it might be a medication
        if len(word) >= 6 and word.isalpha():
            # Additional heuristics for medication-like words
            vowel_count = sum(1 for c in word_lower if c in 'aeiou')
            consonant_count = len(word) - vowel_count
            
            # Medications often have a good balance of vowels and consonants
            if 0.2 <= vowel_count / len(word) <= 0.6:
                return True
